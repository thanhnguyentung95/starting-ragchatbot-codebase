    ...
    def search(
        ...
        try:
            ...
            results = self.course_content.query(
                query_texts=[query], n_results=actual_limit, where=filter_dict
            )
    ...
    def _resolve_course_name(self, course_name: str) -> Optional[str]:
        ...
        try:
            ...
            if results["documents"][0] and results["metadatas"][0]:
                distance = (
                    results["distances"][0][0] if results["distances"][0] else None
                )
                if (
                    distance is not None
                    and distance > self._COURSE_NAME_DISTANCE_THRESHOLD
                ):
                    return None
